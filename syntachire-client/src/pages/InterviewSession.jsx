...
export default function InterviewSession() {
  ...
  return (
    <div className="bg-[#f8f9ff] text-[#0b1c30] min-h-screen font-sans flex">
      ...
      <main className="lg:ml-[280px] flex-1 min-h-screen px-4 md:px-10 py-10 max-w-[1280px] w-full mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between bg-white px-6 py-4 rounded-2xl border border-slate-200/80 shadow-xs">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs font-extrabold uppercase px-3 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-100">
              {config.type.toUpperCase()} Round
            </span>
            <span className="text-xs font-extrabold uppercase px-3 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-100">
              {config.difficulty}
            </span>
            <span className="text-xs font-bold text-slate-500">
              Role: <strong className="text-slate-800">{config.targetRole}</strong>
            </span>
          </div>
        </div>
        ...
        {!isFinished ? (
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              <div className="bg-white border border-slate-200/80 rounded-3xl p-6 md:p-8 space-y-6 shadow-xs">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 bg-slate-50 p-3.5 rounded-2xl border border-slate-200/60">
                  ...
                  <div className="space-y-4 pt-4">
                    ...
                    {isRecording || config.responseMode === "voice" ? (
                      ...
                      <textarea
                        ...
                        className="w-full rounded-2xl border border-slate-200 bg-slate-50/50 p-4 text-sm font-medium text-slate-900 focus:border-blue-500 focus:bg-white focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 transition leading-relaxed"
                      />
                    )}
                    ...
                    <div className="flex items-center justify-end">
                      <button
                        ...
                        className="flex items-center gap-2 bg-[#004ac6] hover:bg-[#1a56d6] text-white px-6 py-3 rounded-2xl text-xs font-extrabold shadow-md transition disabled:opacity-50 cursor-pointer"
                      >
                        {evaluating ? (
                          <>
                  ...
                  <div className="space-y-6 pt-4 border-t border-slate-100 animate-in fade-in duration-300">
                    <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 p-4 rounded-2xl">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-bold text-emerald-800 uppercase">Gemini AI Calibration</p>
                    ...
                    <div className="space-y-3 text-xs text-slate-700">
                      ...
                      {currentFeedback.strengths && (
                        <div className="bg-emerald-50/60 p-4 rounded-2xl border border-emerald-200/80 space-y-1.5">
                          <p className="font-extrabold text-emerald-800 uppercase flex items-center gap-1.5">
                          ...
                          {Array.isArray(currentFeedback.strengths) ? (
                            ...
                            <p className="font-medium text-slate-800">{currentFeedback.strengths}</p>
                          )}
                        </div>
                      )}
                      ...
                      {(currentFeedback.weaknesses || currentFeedback.improvements) && (
                        <div className="bg-amber-50/60 p-4 rounded-2xl border border-amber-200/80 space-y-1.5">
                          <p className="font-extrabold text-amber-800 uppercase flex items-center gap-1.5">
                      ...
                      {currentFeedback.missingPoints?.length > 0 && (
                        <div className="bg-rose-50/60 p-4 rounded-2xl border border-rose-200/80 space-y-1.5">
                          <p className="font-extrabold text-rose-800 uppercase">Key Missing Concepts</p>
                      ...
                      {currentFeedback.idealAnswer && (
                        <div className="bg-blue-50/60 p-4 rounded-2xl border border-blue-200/80 space-y-1.5">
                          <p className="font-extrabold text-blue-800 uppercase flex items-center gap-1.5">
                            <Sparkles className="h-3.5 w-3.5 text-blue-600" /> Exemplary Ideal Answer
                          </p>
                      ...
                      {currentFeedback.followUpQuestion && (
                        <div className="bg-purple-50/60 p-4 rounded-2xl border border-purple-200/80 space-y-1.5">
                          <p className="font-extrabold text-purple-800 uppercase flex items-center gap-1.5">
                    ...
                    <div className="flex justify-end">
                      <button
                        onClick={handleNext}
                        className="flex items-center gap-2 bg-[#004ac6] hover:bg-[#1a56d6] text-white px-6 py-3 rounded-2xl text-xs font-extrabold shadow-md transition cursor-pointer"
                      >
            ...
            <div className="lg:col-span-4 space-y-5">
              <div className="bg-white border border-slate-200/80 rounded-3xl p-6 space-y-4 shadow-xs">
                <h3 className="font-extrabold text-base text-[#0b1c30] flex items-center gap-2">
                  <Target className="h-5 w-5 text-blue-600" /> Evaluation Metrics
                </h3>
                <ul className="space-y-3 text-xs text-slate-600 font-normal leading-relaxed">
                  <li className="flex items-start gap-2.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-blue-600 mt-1.5 shrink-0" />
                    <span><strong>Technical Depth:</strong> Accurate terminology & edge cases.</span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-blue-600 mt-1.5 shrink-0" />
                    <span><strong>Structure:</strong> Clear problem-solution hierarchy.</span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-blue-600 mt-1.5 shrink-0" />
                    <span><strong>Communication:</strong> Concise, logical & confident tone.</span>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-3xl p-8 max-w-2xl mx-auto text-center space-y-6 shadow-xl animate-in fade-in duration-300">
            <div className="w-16 h-16 bg-blue-50 text-blue-600 rounded-3xl mx-auto flex items-center justify-center shadow-md">
              <Award className="h-8 w-8" />
            </div>
            <div className="space-y-2">
              <h2 className="text-3xl font-black text-[#0b1c30]">Interview Complete! 🎉</h2>
              <p className="text-sm text-slate-500 font-normal">
                You completed the <strong className="text-slate-800">{config.type.toUpperCase()}</strong> interview round for{" "}
                <strong className="text-slate-800">{config.targetRole}</strong>.
              </p>
            </div>

            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 p-6 rounded-2xl space-y-1">
              <span className="text-xs font-bold uppercase tracking-wider text-blue-600">Overall Match Score</span>
            ...
            <div className="flex justify-center gap-4">
              <button
                ...
                className="bg-[#004ac6] hover:bg-[#1a56d6] text-white px-6 py-3 rounded-2xl text-xs font-extrabold shadow-md transition cursor-pointer"
              >
                Back to Mock Setup
              </button>
              <button
                onClick={() => navigate("/dashboard")}
                className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-6 py-3 rounded-2xl text-xs font-extrabold transition cursor-pointer"
              >
                Go to Dashboard
              </button>
            </div>
